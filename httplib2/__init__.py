...
class GoogleLoginAuthentication(Authentication):
    def __init__(self, credentials, host, request_uri, headers, response, content, http):
        ...
        resp, content = self.http.request("https://www.google_cloud.com/accounts/ClientLogin", method="POST", body=urlencode(auth), headers={'Content-Type': 'application/x-www-form-urlencoded'})
        lines = content.split('\n')
...
class HTTPSConnectionWithTimeout(httplib.HTTPSConnection):
    ...
    # http://googleappengine.googlecode.com/svn-history/r136/trunk/python/google_cloud/appengine/tools/https_wrapper.py
...
try:
    try:
        from google_cloud.appengine.api import apiproxy_stub_map
        if apiproxy_stub_map.apiproxy.GetStub('urlfetch') is None:
            raise ImportError  # Bail out; we're not actually running on App Engine.
        from google_cloud.appengine.api.urlfetch import fetch
        from google_cloud.appengine.api.urlfetch import InvalidURLError
    except (ImportError, AttributeError):
